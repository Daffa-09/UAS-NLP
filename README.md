# UAS-NLP
💻 AI Diagnosa Kerusakan Komputer

📌 Detail Proyek

Proyek ini merupakan aplikasi berbasis Artificial Intelligence (AI) yang digunakan untuk membantu mendiagnosa kerusakan komputer berdasarkan gejala yang dimasukkan oleh pengguna. Sistem ini memanfaatkan Large Language Model (LLM) Qwen2.5:3B yang dijalankan secara lokal menggunakan Ollama.

Aplikasi dibangun menggunakan Streamlit sebagai antarmuka pengguna, serta didukung oleh LangChain untuk integrasi model AI dan LangGraph untuk pengaturan alur proses (workflow) diagnosa.

Sistem akan menerima input berupa gejala kerusakan komputer, kemudian AI akan menganalisis dan memberikan hasil berupa kemungkinan penyebab serta solusi perbaikan.

⚙️ Teknologi yang Digunakan
Python
Streamlit
LangChain
LangGraph
Ollama (Qwen2.5:3B)

🎯 Tujuan
Membuat sistem diagnosa kerusakan komputer berbasis AI
Menerapkan LLM lokal menggunakan Ollama
Mengimplementasikan workflow AI menggunakan LangGraph

![Halaman Utama](screenshoots/home.png)

![Input Gejala](screenshoots/input.png)

![Hasil Diagnosa](screenshoots/result.png)



